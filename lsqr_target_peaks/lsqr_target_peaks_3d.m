...
Etheta_targ = [];
Ephi_targ = [];
phi_target_idx_range = 1:1:numel(phi_range);
if phi_target_idx_range(end) < numel(phi_range)
    phi_target_idx_range(end+1) = numel(phi_range);
end
for phi_idx = phi_target_idx_range
...
function update_phi(phi_val, dish_analyzer, EdB, EdB_sum, THETA, PHI, theta_range, h1, h2, h3)
    ...
    disp(rad2deg(dish_analyzer.get_beam_width(phi_val,EdB_sum_phi,theta_range)));
