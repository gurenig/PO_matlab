...
figure;
theta_shift = -180;
plot(rad2deg(theta_range) + theta_shift, EdB);
hold on;
plot(rad2deg(theta_range) + theta_shift, EdB_sum, 'LineStyle', '-');
...
xlabel("\theta' [deg]");
xlim([min(rad2deg(theta_range)), max(rad2deg(theta_range))] + theta_shift);
yl = ylim;
